...
int main(int argc, char *argv[])
{
    ...
    //Create a DB
    QFile::remove(a.applicationDirPath() + "/UniqueSavingBug.db3");
    QxSqlDatabase::getSingleton()->setDriverName(QStringLiteral("QSQLITE"));
    QxSqlDatabase::getSingleton()->setDatabaseName(a.applicationDirPath() +
                                                   "/UniqueSavingBug.db3");
    ...
    query.exec(QStringLiteral("CREATE TABLE t_Type (Type_id INTEGER NOT NULL PRIMARY KEY, Name varchar, "
        "Def BLOB);"));
    ...
    query.exec(QStringLiteral("CREATE TABLE t_MetaData (MetaData_id INTEGER NOT NULL PRIMARY KEY, "
        "MetaKey INTEGER NOT NULL, Value varchar, UNIQUE (MetaKey, Value), "
        "FOREIGN KEY (MetaKey) REFERENCES t_MetaDataKey (MetaDataKey_id) "
            "ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED);"));
    query.exec(QStringLiteral("CREATE TABLE t_qxee_KPI_MetaData (KPIId INTEGER, MetaData_id INTEGER, "
        "FOREIGN KEY (KPIId) REFERENCES t_KPI (KPI_id) "
            "ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED, "
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        m.getLast()->setMetaKey(k); m.getLast()->setValue(QString::number(i));
    }
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        v.at(i)->list_of_MetaData().insert(m.getLast()->getMetaData_id(), m.getLast());
        v.at(i)->setTypeId(t);
        v.at(i)->setValue(i);
    }
